...
/** Statuses the timeout clock applies to (the daily cron only reaps these). */
export const INVOICE_REQUEST_TIMEOUT_STATUSES = ["open", "contacted"] as const;

/**
 * Whole days until an active request hits the timeout cliff (holds released,
 * suggested lots deactivated). 0 = times out on the next daily cron run;
 * negative = already past the cliff. Null when the clock doesn't apply.
 */
export function invoiceRequestDaysLeft(
  createdAt: string | null | undefined,
  status?: string,
): number | null {
  if (
    status !== undefined &&
    !(INVOICE_REQUEST_TIMEOUT_STATUSES as readonly string[]).includes(status)
  ) {
    return null;
  }
  ...
  return Math.ceil((t + INVOICE_REQUEST_TIMEOUT_DAYS * dayMs - Date.now()) / dayMs);
}
