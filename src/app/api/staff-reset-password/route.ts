...
import { setStaffPasswordForce } from "@/lib/firestore/staff";
...
function redirectWithError(request: Request, token: string, message: string) {
  const res = NextResponse.redirect(
    new URL(
      `/wholesaleportal/reset-password?token=${encodeURIComponent(token)}&error=${encodeURIComponent(message)}`,
...
export async function POST(request: Request) {
  ...
  if (!record || record.role !== "staff") {
    return redirectWithError(request, token, "This reset link has expired or was already used.");
  }

  const result = await setStaffPasswordForce(record.accountId, password);
  if (!result.ok) {
  ...
  const res = NextResponse.redirect(
    new URL(
      `/wholesaleportal/sign-in?ok=${encodeURIComponent("Password updated — sign in with your new password.")}`,
      publicOrigin(request),
    ),
    303,
  );
